...
These are incremental building blocks toward `USNativeDDRPHY`. The package has
pin extraction, physical mapping, RXTX/control/tristate emitters and a native RIU helper.
A complete upstream PHY and portable standalone calibration implementation
remain pending. Complete-system KCU116 builds have passed normal implementation
outside this checkout, as has a VCU118 x64 / 512-bit candidate at 2400 MT/s.
These wider systems are not hardware qualified.
Existing PHY implementations and defaults are unchanged.
...
execution and must reject a failed Vivado run; the library does not start a
process or silently load cached results.
...
These helpers do not modify existing PHY defaults or instantiate a complete
PHY/calibration stack. Local controller, cache, CPU-reset, DMA benchmark and
Vivado-strategy changes are deliberately outside this package. Original
UltraScale/UltraScale+ eligibility and physical topology checks still apply;
