...
class NativeDMABenchmark(LiteXModule):
    def __init__(self, write_port, read_port, capacity=0x40000000, fifo_depth=64, drained=None, databits=32):
        ...
        base=Signal(32);beat_count=Signal(32);last_beat=Signal(32,reset=0xffffffff);random_data=Signal();read_only=Signal();timeout=Signal(32)
        write_index=Signal(32);read_index=Signal(32);received=Signal(32)
        write_cycles=Signal(32);read_cycles=Signal(32)
        write_beats=Signal(32);read_beats=Signal(32)
        write_stalls=Signal(32);read_stalls=Signal(32)
        errors=Signal(32);first_offset=Signal(32);first_xor=Signal(width)
        dq_errors=Signal(databits)
        self.comb += self._dq_error_mask.status.eq(dq_errors)
        fault=Signal(3);settle=Signal(8)
        mismatch=Signal();mismatch_offset=Signal(32);mismatch_xor=Signal(width)
