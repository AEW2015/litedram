...
class DMATest(unittest.TestCase):
    def test_one_beat_per_cycle_with_outstanding_reads(self):
        wp=LiteDRAMNativePort('write',10,128);rp=LiteDRAMNativePort('read',10,128)
        top=Module();top.submodules.dut=dut=NativeDMABenchmark(wp,rp,capacity=16384)
        memory=Memory(128,1024);wr=memory.get_port(write_capable=True);rd=memory.get_port(async_read=True)
        ...
        pipeline=[Signal(128) for _ in range(16)];valid=[Signal() for _ in range(16)]
        top.sync += [pipeline[0].eq(rd.dat_r),valid[0].eq(rp.cmd.valid & rp.cmd.ready)]
        for i in range(1,16): top.sync += [pipeline[i].eq(pipeline[i-1]),valid[i].eq(valid[i-1])]
        top.comb += [rp.rdata.data.eq(pipeline[-1]),rp.rdata.valid.eq(valid[-1])]
        def main():
            yield dut.allowed.eq(1);yield dut._base.storage.eq(0);yield dut._length.storage.eq(4096)
            yield dut._start.re.eq(1);yield;yield dut._start.re.eq(0)
            for _ in range(2000):
    ...
    def run_case(self,random_data=1,corrupt=False,width=128):
        ...
        def start(read_only=0):
            ...
            yield dut._start.re.eq(1);yield;yield dut._start.re.eq(0);yield
        def wait():
            for _ in range(10000):
                if (yield dut._done.status): return
                yield
            self.fail('DMA did not finish')
        def main():
            yield dut.allowed.eq(1);yield dut._base.storage.eq(64)
            yield dut._length.storage.eq(1024);yield dut._random.storage.eq(random_data)
            yield from start();yield from wait()
            self.assertEqual((yield dut._fault.status),0)
            self.assertEqual((yield dut._errors.status),0)
            ...
            if corrupt: memory.mem[last]^=1<<100  # final beat must also be checked
            yield from start(1);yield from wait()
            self.assertEqual((yield dut._fault.status),0)
            ...
            yield dut._length.storage.eq(0);yield from start();yield from wait()
            self.assertEqual((yield dut._fault.status),1)
            self.assertEqual((yield dut._write_beats.status),0)
            yield dut._length.storage.eq(beat_bytes);yield from start();yield from wait()
            self.assertEqual((yield dut._fault.status),0)
            self.assertEqual((yield dut._errors.status),0)
    ...
    def test_counter_backpressure(self): self.run_case(0)
    def test_prbs_backpressure_and_corruption(self): self.run_case(1,True)
    def test_256_bit_counter(self): self.run_case(0,width=256)
    def test_256_bit_prbs_corruption(self): self.run_case(1,True,width=256)
    def test_reject_and_timeout(self):
        ...
        def main():
            yield dut._base.storage.eq(0);yield dut._length.storage.eq(16)
            yield from start();self.assertEqual((yield dut._fault.status),2)
            yield dut.allowed.eq(1);yield dut._base.storage.eq(16384)
            yield from start();self.assertEqual((yield dut._fault.status),1)
            yield dut._base.storage.eq(1)
            yield from start();self.assertEqual((yield dut._fault.status),1)
            yield dut._base.storage.eq(0xfffffff0);yield dut._length.storage.eq(32)
            yield from start();self.assertEqual((yield dut._fault.status),1)
            yield dut._length.storage.eq(16)
            yield dut._base.storage.eq(0);yield dut._timeout.storage.eq(16)
            yield from start()
            for _ in range(32): yield
            self.assertEqual((yield dut._fault.status),3)
            self.assertEqual((yield dut._done.status),1)
            self.assertEqual((yield dut._busy.status),0)
            yield from start();self.assertEqual((yield dut._fault.status),3)
        simulate(dut,main())
    ...
    def test_lost_readiness_is_fatal_and_cannot_restart(self):
        ...
        def main():
            ...
            yield dut._base.storage.eq(0);yield dut._length.storage.eq(4096)
            yield dut._start.re.eq(1);yield;yield dut._start.re.eq(0)
