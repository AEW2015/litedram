...
class RIUTest(unittest.TestCase):
    ...
    def test_transactions(self):
        ...
        def driver():
            ...
            yield dut.select.eq(1);yield dut.address.eq(0x30);yield dut.wdata.eq(0xbeef);yield dut.write.eq(1)
            yield dut.request.eq(1);yield;yield dut.request.eq(0);yield
            self.assertEqual((yield dut.busy),1)
            yield dut.wdata.eq(0x9999);yield dut.address.eq(7);yield dut.select.eq(2)
            for _ in range(4):yield
            yield dut.request.eq(1);yield;yield dut.request.eq(0)
            ...
            yield dut.write.eq(0);yield dut.request.eq(1);yield;yield dut.request.eq(0);yield
            self.assertEqual((yield dut.valid),0)
            for _ in range(100):yield
            self.assertEqual((yield dut.rdata),0xabcd);self.assertEqual((yield dut.error),0)
            ...
            yield dut.request.eq(1);yield;yield dut.request.eq(0);yield dut.reset.eq(1)
            for _ in range(100):yield
            yield dut.reset.eq(0)
            for _ in range(20):yield
            self.assertEqual((yield dut.busy),0);self.assertEqual((yield dut.valid),0)
    ...
    def test_waits_for_native_ready(self):
        ...
        def driver():
            yield dut.write.eq(1);yield dut.wdata.eq(0xbeef);yield dut.request.eq(1)
            yield;yield dut.request.eq(0)
            for _ in range(30):
                yield
                self.assertEqual((yield dut.native_write),0)
            self.assertEqual((yield dut.busy),1)
            yield dut.native_valid.eq(3);yield dut.native_rdata.eq(0xbeef)
            high=0
            for _ in range(100):
                yield
    ...
    def test_timeout_invalid(self):
        ...
        def driver():
            yield dut.request.eq(1);yield;yield dut.request.eq(0)
            for _ in range(100):yield
            self.assertEqual((yield dut.busy),0);self.assertEqual((yield dut.valid),0);self.assertEqual((yield dut.error),1)
            yield dut.select.eq(3);yield dut.request.eq(1);yield;yield dut.request.eq(0)
            for _ in range(50):yield
            self.assertEqual((yield dut.busy),0);self.assertEqual((yield dut.valid),0);self.assertEqual((yield dut.error),1)
            # A valid request after timeout/rejection must recover normally.
            yield dut.select.eq(0);yield dut.native_valid.eq(1);yield dut.native_rdata.eq(0x6543)
            yield dut.request.eq(1);yield;yield dut.request.eq(0)
            for _ in range(80):yield
            self.assertEqual((yield dut.valid),1);self.assertEqual((yield dut.error),0)
